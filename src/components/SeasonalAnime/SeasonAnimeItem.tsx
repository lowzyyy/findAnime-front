...
import { addUnderscore, formatNumbers } from "../../helpers/globalHelpers";

function SeasonAnimeItem({ animeData }: { animeData: IAnime }) {
  const members = formatNumbers(animeData.members);
  return (
    <li
    ...
    >
      <a
        href={`/anime/${animeData.mal_id}/${addUnderscore(
      ...
      >
        ...
        >
          {[...animeData.genres, ...animeData.explicit_genres].map(
            (el, index) => (
              <span
                title={`${el.name}`}
                className="rounded-md bg-[#37f8ee42]"
                key={index}
              >
                {el.name}
              </span>
